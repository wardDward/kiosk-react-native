...
const ViewOrder = ({navigation}) => {
  ...
  const checkOut = async () => {
    ...
    if (paymentStyle === '') {
      Alert.alert('Alert Title', 'Type Of Payment', [
        {
          text: 'Cash',
  ...
  const checkOutItem = async () => {
    try {
      const response = await axios.post(
        'http://192.168.100.7:5000/api/orders/cashier',
        {
          orders: orders,
          type: type,
      ...
      paymentAction(orderId, paymentStyle, total_price);
  ...
  const paymentAction = async (orderId, paymentStyle, total_price) => {
    try {
      await axios.post('http://192.168.100.7:5000/api/orders/payment', {
        orderId: orderId,
